...
export default function EditProduct() {
    ...
    async function fetchProduct() {
        try {
            ...
            setUnitPrice(data.productPrice?.toString() || "");
            setIsControlled(data.isControlled || false);
            setCategoryId(data.categoryId?.toString() || "");
            setTypeId(data.productTypeId?.toString() || "");
            setSupplierId(data.supplierId?.toString() || "");

            // Load product image
            if (data.productImage) {
                const image = `data:image/jpeg;base64,${data.productImage}`;
    ...
    // Update supplier/category queries when data loads
    useEffect(() => {
    ...
    useEffect(() => {
        if (categoryId && categoryOptions.length > 0) {
            const selectedCategory = categoryOptions.find(c => c.value.toString() === categoryId.toString());
            if (selectedCategory) {
                setCategoryQuery(selectedCategory.categoryName);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setErrorMessage(errorText || "Failed to update product.");
                setSuccessMessage("");
            }
        } catch (err) {
    ...
    if (loading) {
        return (
            <div className="add-product-page">
                <FormHeader title="Edit Product Record" to="/products" />
    ...
    return (
        <div className="add-product-page">
            ...
            <FormWrapper title="Edit Product Details:">
